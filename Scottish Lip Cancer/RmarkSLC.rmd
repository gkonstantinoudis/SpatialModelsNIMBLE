...
{\bf \phi} & \sim & \text{ICAR}({\bf W}, 1/\tau_{\phi})
\end{eqnarray}
\]
...
BYM2 is a reparametrization of BYM model which has the following features: 1) It resolves the identifiability issues of the BYM model 2) The hyperparameters of the field are almost orthogonal, facilitating clear interpretation and making prior assignment intuitive, 3) The spatially structured random effect is scaled and thus the hyperprior used in one graph have the same interpretation in any other graph (for instance the hyperpriors for the graph of Scotland will have same interpretation as the hyperpriors for the graph of Greece only if the spatially structured random effect is scaled). For more information see @riebler2016 and @freni2018.
\[
\begin{eqnarray}
O_{i}|\lambda_{i}  & \sim & \text{Poisson}(\lambda_{i}E_{i} )  \\
...
data.frame(model = c( rep("Unstr: theta", N), 
                     rep("ICAR: phi", N), rep("BYM: phi", N), rep("BYM2: b", N)),
           field =c(UnstrCodesamples$summary$all.chains[paste0("theta[", 1:56, "]"), "Median"],
                    ICARCodesamples$summary$all.chains[paste0("phi[", 1:56, "]"), "Median"],
                    # BYMCodesamples$summary$all.chains[paste0("theta[", 1:56, "]"), "Median"] + 
                      BYMCodesamples$summary$all.chains[paste0("phi[", 1:56, "]"), "Median"],
                    BYM2Codesamples$summary$all.chains[paste0("b[", 1:56, "]"), "Median"])) %>% 
  
  ggplot() + geom_boxplot(aes(x = model, y = field)) + theme_bw() + 
  theme(text = element_text(size = 14)) + ggtitle("Medians of the field")


data.frame(model = c( rep("Unstr: theta", N), 
                      rep("ICAR: phi", N), rep("BYM: phi", N), rep("BYM2: b", N)),
           sd =c(UnstrCodesamples$summary$all.chains[paste0("theta[", 1:56, "]"), "St.Dev."],
                    ICARCodesamples$summary$all.chains[paste0("phi[", 1:56, "]"), "St.Dev."],
                    # BYMCodesamples$summary$all.chains[paste0("theta[", 1:56, "]"), "St.Dev."] + 
                      BYMCodesamples$summary$all.chains[paste0("phi[", 1:56, "]"), "St.Dev."],
                    BYM2Codesamples$summary$all.chains[paste0("b[", 1:56, "]"), "St.Dev."])) %>% 
  
  ggplot() + geom_boxplot(aes(x = model, y = sd)) + theme_bw() + 
  theme(text = element_text(size = 14)) + ggtitle("SDs of the field")

```

# Conclusion
The purpose of this tutorial is to show how to fit spatial models using `NIMBLE` [@nimble]. Nevertheless, the tutorial does not provide an exhaustive list of spatial model that can be fit in `NIMBLE`. Examples of other models include the proper model [@cressie1989], the Leroux model [@leroux2000], see also @best2005 and @lee2011 for simulation studies and relevant comparisons. Additional sources for code include the tutorial by Lawson [@lawson2020nimble] and also chapter 9 of [the NIMBLE manual](https://r-nimble.org/html_manual/cha-welcome-nimble.html).
