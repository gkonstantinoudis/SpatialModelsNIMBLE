---
title: "Leroux models in NIMBLE and CARBayes: An example with the Scottish lip cancer data"
...
params <- c("sigma2.theta", "SIR", "alpha", "rho", "theta")
...
Using the same number of iterations in NIMBLE, we did not reach convergence, we should change the MCMC setting and rerun the 

```{r eval=FALSE}

ni <- 5000000  # nb iterations 
nt <- 400      # thinning interval
...
t_1 <- Sys.time()
t_largesetting <- t_1 - t_0

saveRDS(Lerouxsamples, file = "LerouxsamplesLargeSetting")
...
Lerouxsamples <- readRDS("LerouxsamplesLargeSetting")
```

* Convergence diagnostics as before.
```{r fig.height=4, fig.width=9}
...
From the first look, CARBayes needs less time (14 sec) and samples to converge compared to the NIMBLE specification shown in this tutorial (36 minutes). We should consider the current differences in the samplers, for instance CARBayes uses block sampling for the random effects, whereas in our NIBLE specification we do not have block sampling. True is you can employ block sampling in `NIBLE` if needed.
...
Although it seems that the variance and mixing parameter look very similar, the variance of the intercepts is very different.
...
densplot(chain1$samples$beta, las = 1, main = "Chain 1", 
         xlim = c(-2.5,2.5), col = "#ff9999")

densplot(chain2$samples$beta, col = "#33cccc", 
         las = 1, main = "Chain 2", xlim = c(-2.5,2.5))
...
As expected the densities of the intercepts are quite different, with respect to the variation.
...
dat.points <- data.frame(CARBayes = apply(chain1$samples$phi, 2, median), 
                         NIMBLE = 
                         Lerouxsamples$summary$all.chains[paste0("theta[", 1:nrow(data.combined), 
                                                                 "]"), "Median"])
...
dat.box <- data.frame(sd = c(Lerouxsamples$summary$all.chains[paste0("theta[", 
...
This tutorial is a first attempt to fit the Leroux model in `NIMBLE`. We compared the results with the `CARBayes` package which is the standard when it comes to fitting the Leroux model. The results are overall similar, with great discrepancy on the intercepts. The variation of the intercept is then inherited to the latent fields. Trying to understand the reason between such discrepancies in the variance of the intercept is work in progress. Any ideas are more than welcome :) 
